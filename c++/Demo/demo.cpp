 ...
 * ext: extension of output file's name; also indicates to use approximation or not
 *
 * output:
 ...
 */
{
    using namespace std::chrono; // for clock and time
    using namespace cv;
    ...
    // Display the score image
    {
        ...
        minMaxLoc(*score, &minVal, &maxVal); // from cv: finds global min&max and/or their location
        double range = maxVal;
        *score = (*score) / range * 255;
        Mat temp;
        score->convertTo(temp, CV_8U); // convert data type to do imwrite
        cv::imwrite(buf,temp);
    }   
